...
def ticks_to_bars(tick_data, interval):
    ...
    while tick_index < tick_data.size:
        ...
        if interval_end == tick_index:
            ...
            next_entry[0]['per_vlm'] = np.sum(tick_data[tick_index:interval_end]['volume'])
...
def create_commodity_bars(h5_file_path, start_date, end_date, duration, start_time, end_time):
    with tables.open_file(h5_file_path, mode='r') as h5file:
        for node in h5file.walk_nodes("/", classname='Table'):
            ...
            resampled = df.resample(duration).agg({
                'price': 'ohlc',
                'volume': 'sum'
            })
...
def main():
    ...
    create_commodity_bars(h5_file_path, start_date, end_date, duration, start_time, end_time)
