...
class CommodityTicks:
    ...
    def __init__(self, sym, name, h5file, h5path, is_live):
        ...
        if (h5path + '/' + self.name not in self.h5file):
            self.h5table = h5file.create_table(h5path, self.name, H5TickType, "{} table".format(self.name))
        else:
    ...
    def process_file(self, path):
        ...
        for row in data.itertuples(index=False, name='Commodity'):
            ...
            next_entry = np.array([(np_date, np_date - np_date.astype('M8[D]'), row.LastP, row.LastV)], dtype=tick_type)
...
def make_commodity_data(h5file):
    commodities = {}
    commodities["WC"] = CommodityTicks("WC", "soft_red_wheat", h5file, "/HistTicks", False)
    ...
    commodities["SM"] = CommodityTicks("SM", "soybean_meal", h5file, "/HistTicks", False)
